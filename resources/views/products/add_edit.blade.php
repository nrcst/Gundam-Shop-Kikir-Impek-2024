...
<div style="display: flex; flex-direction: column; align-items: center; background-color: #fef8f0; padding: 20px;">
    <form action="{{ isset($product) ? route('products.update', $product->id) : route('products.store') }}"
        method="POST" style="width: 100%; max-width: 800px;">
        @csrf
        @if(isset($product))
            @method('PUT')
        @endif
        <div style="background-color: #fff2e0; width: 100%; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            ...
            <div style="margin-bottom: 10px;">
                ...
                <input type="text" id="productName" name="name" value="{{ old('name', $product->name ?? '') }}"
                    style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px;">
            </div>
            <div>
                <label for="description" style="display: block; font-weight: bold;">Description</label>
                <textarea id="description" name="description"
                    style="width: 100%; height: 200px; resize: none; padding: 10px; border: 1px solid #ccc; border-radius: 5px;"
            ...
            <div style="background-color: #fff2e0; flex: 1; padding: 20px; border-radius: 10px;">
                ...
                <div>
                    ...
                    <input type="number" id="price" name="price" value="{{ old('price', $product->price ?? '') }}"
                        step="0.01" style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px;">
                </div>
            </div>
            <div style="background-color: #fff2e0; flex: 1; padding: 20px; border-radius: 10px;">
        ...
        <div style="text-align: center;">
            <button type="submit"
                style="background-color: #5a2d0c; color: white; padding: 10px 20px; border: none; border-radius: 5px;">{{ isset($product) ? 'Update Product' : 'Add Product' }}</button>
        </div>
    </form>
</div>
